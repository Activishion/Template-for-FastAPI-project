...
from users.utils import get_password_hash, is_valid_password
from users.schemas import (RefreshSessionUpdate, UserCreate, User, Token, UserCreateDB,
                           RefreshSessionCreate, UserUpdate, UserUpdateDB)
    ...
    async def refresh_token(cls, token: uuid.UUID) -> Token:
        async with async_session_maker() as session:
            ...
            time_difference_check = (
                refresh_session.created_at + timedelta(seconds=refresh_session.expires_in)
            )
            if datetime.now(timezone.utc) >= time_difference_check:
